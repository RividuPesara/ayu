...
async def list_articles(user: CurrentUser = Depends(require_patient_access),
) -> ArticleListResponse:
    articles = await _run_sync(get_published_articles)
    return ArticleListResponse(articles=articles, total=len(articles))

@router.get("/{article_id}", response_model=ArticleResponse)
async def get_article(article_id: str,user: CurrentUser = Depends(require_patient_access),) -> ArticleResponse:
